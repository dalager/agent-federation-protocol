...
export class DeliveryQueue {
  ...
  enqueue(target: string, activity: { [key: string]: JsonValue }, now = new Date()): number {
    const info = this.db.run(
        `INSERT INTO delivery_queue
         ...
         VALUES (?, ?, 0, ?, 'pending', ?, ?)`,
        String(activity.id ?? ""), target, now.getTime(), now.toISOString(), JSON.stringify(activity));
  ...
  ready(now = new Date()): QueueItem[] {
    const rows = this.db.all(
        `SELECT * FROM delivery_queue
          ...
          ORDER BY id ASC`,
        now.getTime()) as Record<string, unknown>[];
    ...
    // The SQL here is deliberately static, with no `IN (?,?,…)` built from
    // the target count. `ready()` is the delivery loop's polling call, and
    // the store port caches prepared statements by SQL text — a placeholder
    // list sized by however many peers happen to be pending would mint one
    // cached statement per distinct cardinality, retained for the life of
    // the process. `peer_backoff` holds at most one row per peer, so reading
    // it whole and filtering in memory is both smaller and fixed-shape.
    const blocked = new Map(
      (this.db.all("SELECT target, not_before FROM peer_backoff") as { target: string; not_before: number }[]).map(
        (row) => [row.target, Number(row.not_before)],
      ),
    );
  ...
  peerNotBefore(target: string): number | null {
    const row = this.db.get("SELECT not_before FROM peer_backoff WHERE target = ?", target) as
      | { not_before: number }
      | undefined;
  ...
  async flush(transport: Transport, now = new Date()): Promise<DeliveryReport> {
    ...
    for (const item of this.ready(now)) {
      ...
      try {
        ...
        this.db.run("UPDATE delivery_queue SET state = 'delivered', attempts = ? WHERE id = ?", attempts, item.id);
        report.delivered++;
      } catch (error) {
        ...
        if (attempts >= this.maxAttempts) {
          this.db.run("UPDATE delivery_queue SET state = 'dead', attempts = ?, last_error = ? WHERE id = ?", attempts, message, item.id);
          ...
          this.db.run(
              `UPDATE delivery_queue
                ...
                WHERE id = ?`,
              attempts, nextAttemptAt, message, item.id);
          report.retried++;
        }
      }
    }
  ...
  stats(): Record<DeliveryState, number> {
    const rows = this.db.all("SELECT state, COUNT(*) AS n FROM delivery_queue GROUP BY state") as { state: string; n: number }[];
