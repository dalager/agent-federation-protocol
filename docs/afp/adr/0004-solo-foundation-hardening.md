...
|---|---|---|
| **H14** | Constrain who may issue an `afp:Enroll` for a hub — [ADR-0005](0005-operators-are-equal.md) | The trail is trusted by issuer today, so a rostered agent can self-promote. The hole predates this ADR — `enrolled_members` had the identical model — but Decisions 1–3 now hang bid admission, announce authority, asset registration and quorum pinning off it, so the blast radius is materially larger. Fixing it is an authority decision, not a hardening detail |
