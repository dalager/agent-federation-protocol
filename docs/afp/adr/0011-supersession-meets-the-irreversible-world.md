...
| ID | Task | Where | Stage |
|---|---|---|---|
| **Y1** ✅ | `afp:irrevocableActions` as the fourth pin: `TaskPins` (`ap/pins.ts:29`) and `buildPinSet` (`ap/pins.ts:78`); `PIN_KEYS` (`pins.py:25`) — the digest and equality paths need no change, they iterate the tuple; a `check_pins` block validating every declared name is a value of the pinned policy, beside the existing `afp:no-verdict` block (`pins.py:156`) | `ap/pins.ts`, `pins.py` | 1 |
