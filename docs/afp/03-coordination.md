...
| `afp:no-decision`, `afp:noDecisionReason` | Reserved outcome value; property (DecisionRecord) | A round that ran out of time or fell short still closes with a DecisionRecord: `afp:outcome: "afp:no-decision"` with a reason — `expired` \| `threshold-not-met` \| `quorum-impossible` — each recomputable from the proposal and the votes. Terminal and inert: ratifies nothing, supersedes nothing, and releases an actuator only through the policy's reserved `afp:no-decision` key. MUST NOT appear in `afp:options` (ADR-0018 Decision 2, ADR-0019, ADR-0020 Decision 4) |
