...
| `afp:KeyCompromiseClaim` | `Create`-carried object, on the claiming instance's own chain | The convicted agent's own instance saying its key was captured: names the proof it answers (`afp:proof`, an activity digest), the key it says was captured (`afp:verificationMethod`), and the instant from which it claims the capture ran (`afp:since`). Changes nothing automatically — zeroing stays automatic — and exists so the record can tell `zeroed` from `zeroed-contested` (ADR-0021 Decision 4a) |
| `afp:governanceSubject` / `afp:excluded` | Properties on `afp:Proposal` | The agent a governance round is about; and every member-role seat the round did not pin, each `{agent, afp:status}` with the status from a closed set (`not-live` \| `not-pinned` \| `recused`) and, for a recusal, an `afp:cause` the record resolves — `{afp:form: "equivocation-proof", afp:proof}` naming a proof on record convicting this agent, or `{afp:form: "governance-subject"}` (ADR-0021 Decisions 2-3) |
...
| `afp:Settlement` | Activity | Links prior estimates to observed actuals, releasing deferred reputation adjustment (04) — or, with `afp:decision` as its subject, links a DecisionRecord to what the world showed (ADR-0018 Decision 4; the two subjects are mutually exclusive) |
...
| `afp:role` | Property (`afp:Enroll`) | `member` \| `requester` \| `observer` \| `actuator` — participation scope, enforced at bid admission, snapshot-pinning and actuation (02, ADR-0004, ADR-0019) |
...
| `afp:uncounted` | Property (DecisionRecord) | The pinned voters no vote was counted from, each `declined` (a recorded `Reject` of the proposal — participation without assent) or `silent` (nothing at all, which during a partition is not an abstention). Counted plus uncounted MUST partition the snapshot exactly, and every decline MUST be producible from the bundle (ADR-0014) |
...
| `afp:Proposal` | Object (in `Offer`) | Opens a round (§ 8c): `afp:round`, the closed `afp:options`, and the pinned electorate — `afp:voters`, `afp:voterWeights`, `afp:quorumSnapshot` (the digest of the sorted voter list). Since ADR-0018/0019/0020/0021 it is also where the round's own terms and pins live: `afp:deadline`, `afp:quorumRule`, `afp:binding`, `afp:actionPolicy` and `afp:irrevocableActions` (the proposal is a pin site), `afp:level`, `afp:successionRule`, `afp:supersedesRound`, `afp:excluded`, `afp:governanceSubject`. Every one is optional and emitted only when supplied, so a proposal written before those decisions is byte-identical |
| `afp:deadline` | Property (`afp:Task`, `afp:Proposal`) | An RFC 3339 instant, milliseconds and `Z`. On a task, the bar the delegator's sweep enforces (05); on a proposal, when the round expires — a round past it closes as `afp:no-decision` (`expired`), never abandons (ADR-0018 Decision 2) |
| `afp:quorumRule` | Property (`afp:Proposal`) | `{afp:form, …}` from a closed registry, over `T` = the sum of the pinned weights: `majority-of-total` (`floor(T/2) + 1`), `two-thirds-of-total` (`floor(2T/3) + 1`), `explicit` with `afp:threshold` (integer ≥ 1). Recomputed at replay; an unknown form is a replay failure, never a silently ignored field (ADR-0018 Decision 1) |
| `afp:no-decision`, `afp:noDecisionReason` | Reserved outcome value; property (DecisionRecord) | A round that ran out of time or fell short still closes with a DecisionRecord: `afp:outcome: "afp:no-decision"` with a reason — `expired` \| `threshold-not-met` \| `quorum-impossible` \| `electorate-exhausted` — each recomputable from the proposal and the votes. Terminal and inert: ratifies nothing, supersedes nothing, and releases an actuator only through the policy's reserved `afp:no-decision` key. MUST NOT appear in `afp:options` (ADR-0018 Decision 2, ADR-0019, ADR-0020 Decision 4, ADR-0033 Decision 4) |
| `afp:binding` | Property (`afp:Proposal`) | Closed set, currently `"joint"`; absent means advisory. Names the stakes in the same signed object that pins the electorate deciding them, and grants the hub no enforcement power (ADR-0018 Decision 3) |
| `afp:Departure` | Activity (bare `afp:`-typed, shaped like `afp:Settlement`) | A pinned voter's statement, on its own chain and the round's thread, that it is not following a `joint`-binding outcome it was pinned into: `afp:decision` (the DecisionRecord *activity's* digest), `afp:round`, `afp:hub`, a reason in `content`. Replay checks that the decision exists, was declared `joint`, and pinned the departing actor. Visible non-compliance is the mechanism (ADR-0018 Decision 3) |
| `afp:decision`, `afp:observedOutcome`, `afp:dissentVindicated` | Properties (`afp:Settlement` on a decision) | The decision-subject settlement: `afp:decision` (activity digest) replaces `afp:task`/`afp:settles` — exactly one of the two subjects, never both or neither — with `afp:observedOutcome` one of the proposal's `afp:options`, `afp:evidence` in 07's hash-addressed artifact shape, and `afp:dissentVindicated` naming the pinned voters the world proved right (ADR-0018 Decision 4) |
| `afp:Act` | Object (in `Create`) | The object of a decision-actuation: what was done, in `content`, under `afp:actsOn` (the DecisionRecord activity's digest) and `afp:action` = `policy[outcome]`, published by a `member` or `actuator` enrolled in the hub (ADR-0019) |
| `afp:level` | Property (`afp:Proposal`) | `1` pins the L1 round grammar — every vote MUST carry `afp:phase`, `afp:seqNo` and `afp:proposalHash` — so a replay knows which rules to demand; absent means L0. Default policy: a hub whose enrolled members span two or more operators runs L1 (ADR-0020 Decision 1) |
| `afp:successionRule`, `afp:supersedesRound` | Properties (`afp:Proposal`) | The pinned successor rule — `{afp:form: "snapshot-order"}`: rotation through the stalled proposal's own voter list starting after its actor, skipping the convicted and the recorded-silent — and, on the successor round only, the stalled proposal activity's digest. An unentitled successor fails replay by name (ADR-0020 Decision 3) |
| `afp:votes` | Property (`afp:EquivocationProof`) | Exactly two full signed `afp:Vote` activities, verbatim, so the proof recomputes wherever it travels: same (actor, round, phase, seqNo), differing `value` or `afp:proposalHash` (ADR-0020 Decision 2) |
| `afp:ControlTransfer` | Object (in `Create`, on the instance's own chain) | The instance declares who now operates it: `afp:operatedBy` names the new operator, `since` the instant it takes effect. Consulted at snapshot time from the next pinned round on, never retroactively — same class as `Vouch`/`Disown` (ADR-0005 amendment) |
| `afp:evidence`, `afp:priorProofs` | Properties (`afp:Enroll`) | A cited conviction carried verbatim — `{afp:digest, afp:object}`, the full `Announce{afp:EquivocationProof}` — recomputed rather than trusted and required to convict the agent being enrolled; and the falsifiable self-declaration of prior proofs, where `[]` is a signed denial the same case file may contradict (ADR-0021 Decision 5) |
| `afp:frame` | Property (ContributionSummary) | The four things a second party needs to sum the same set, pinned before any arithmetic: `afp:periodRule` (`{afp:form: "hub-observed", afp:hub, afp:from, afp:to}` — a half-open interval between two of the hub's own chain-head digests; wall-clock `afp:period` stays as narration), `afp:inputScope` (`{afp:visibility: […]}`, the classes summed over), `afp:splitRule` (`{afp:form: "declared-shares"}`), `afp:vocabulary` (the revision the inputs were read under). A form the verifier cannot resolve is a named replay failure (ADR-0022 Decision 1) |
...
**The round declares its level.** An L1 round says so on its own proposal — `afp:level: 1`, pinned like everything else on it — so a replay knows which grammar to demand of its votes; absent means L0. Activation is hub policy with the roadmap's trigger as the default: a hub whose enrolled members span two or more operators runs its rounds at L1 ([ADR-0020](adr/0020-p6-hardened-round-stack.md) Decision 1).
...
**A provably doomed round closes early** ([ADR-0020](adr/0020-p6-hardened-round-stack.md) Decision 4). Once zeroing and silence leave no option able to reach the pinned bar — `doomed ⇔ for every option, attainable(option) < bar`, over the pinned weights with the convicted removed — the hub MAY close the round at once, and MUST on any member's demand, as `afp:no-decision` with `afp:noDecisionReason: "quorum-impossible"`; the deadline no longer buys anything but delay. The verifier recomputes the predicate from the proposal and the votes and fails a `quorum-impossible` close whose arithmetic does not hold. A doomed round and a pending one are the same record: nothing is re-tallied, and a round pinning no `afp:quorumRule` cannot be doomed, because doom is meaningless without a bar.
