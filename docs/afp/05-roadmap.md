...
| **P4 — Federation handshake & operator visibility** — **built**, stack in [ADR-0008](adr/0008-p4-federation-stack.md) | `Offer{FederationAgreement}` → countersign → published trust anchor, `afp:MembershipProof`, deny-list and `afp:Defederate`, **payload integrity across the boundary carried by the existing `eddsa-jcs-2022` object proofs, with HTTP Signatures authenticating each hop** (ADR-0008 — no second signature suite), authorized fetch, both halves ([ADR-0013](adr/0013-authorized-fetch.md), built): non-`public` resources return 404 to everyone unentitled, and a signed request from an entitled peer is served — enrollment scoped to locally-hosted hubs until `afp:MembershipProof` exists, `afp:AuditGrant`, dual-publish shadow Notes and the narrow inbound command grammar | Two instances mutually recognize each other; the human window ([ADR-0029](adr/0029-the-human-window-and-the-activitypub-premise.md), built) is a first-class AFP surface — renderings and the command route, both over the read gate — with the Mastodon projection an optional profile behind `AFP_FEDIVERSE_WINDOW`, followable by AFP-aware software and anything that reads a public outbox; delivery to a stock Mastodon inbox is parked (ADR-0023 L18) | A validly-signed activity from an un-agreed instance is hard-rejected; a `parties` activity returns **404, not 403**, to a federated peer that is not named in it. **And the engagement joint-verifies** ([ADR-0009](adr/0009-federated-replay.md), built): both sides' exports replay in one command — one scoped with `afp:Redacted` stubs and a declared omission — passing clean while a deleted stub, tampered received bytes, or a two-story agreement each fail by name |
