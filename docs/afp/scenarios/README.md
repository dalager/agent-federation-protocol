...
| [15](15-the-production-tuesday.md) | 2 · closed + 3 · open | [ADR-0037](../adr/0037-the-served-hub.md) Decisions 1–3 (closes 96, 97) | `npm run demo:served` · `npm run demo:p8` | `adr0031.test.ts`, `adr0032.test.ts`, `adr0033.test.ts`, `adr0034.test.ts`, `adr0026.test.ts`, `adr0035.test.ts`, `adr0037.test.ts` | [8·5·2·0](15-the-production-tuesday.md#coverage-as-of-2026-09-19-after-the-served-surface-got-a-demo) |
...
| 98 | Custody improved by one word: file custody with a passphrase on the same disk | **Narrowed** by [ADR-0035](../adr/0035-remote-custody-and-the-asynchronous-port.md) Decisions 2, 3, 5 (built 2026-09-18): `remote-issued` custody puts the root in a KMS and bounds a stolen host to one short-lived key's interval. Open at the width of Decision 4 — the `remote` mode in which the key never enters host memory — costed and unscheduled |
| 99 | The self-hosted profile needs four things outside the checkout — a proxy, a unit, a backup cron, a key runbook — before a solo practitioner's first federation | **Attached** to [ADR-0036](../adr/0036-the-hosted-profile.md) as a driver — the hosted profile, in which all four are the platform's. Building: WP-1–3 (store port, request port, alarm schedule) built 2026-09-19, WP-4/5 in part; no `actor` adapter, so no operator is spared any of the four yet. Scenario 15 is what it re-walks against |
...
**Triage (2026-09-15, statuses current 2026-09-19).** 96 and 97 are one ADR —
[ADR-0037](../adr/0037-the-served-hub.md), built 2026-09-19 — because the hub a served
instance hosts and the seats its replicas disagree on are the same object. 98 and 99 are
attached as drivers to the ADRs that answer them; both of those have since begun to build,
and both findings stay open at the width of what has not. 100 is accepted as operational and has its checklist line. Held results from the walk — `config check`
